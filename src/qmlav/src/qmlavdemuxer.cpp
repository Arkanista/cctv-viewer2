...
std::atomic<qint64> g_networkBytesAccumulator{0};
...
void QmlAVDemuxer::start()
{
    ...
    m_demuxerThread = QmlAVThread::loop([=]() mutable -> QmlAVLoopController {
        ...
        if (ret >= 0 && avPacket) {
            m_bytesRead.fetch_add(avPacket->size, std::memory_order_relaxed);
            g_networkBytesAccumulator.fetch_add(avPacket->size, std::memory_order_relaxed);
        }
        if (ret < 0) {
            if (ret == AVERROR_EOF) {
